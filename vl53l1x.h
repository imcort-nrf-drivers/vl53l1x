...
void     vl53l1x_startMeasurement(uint8_t offset); //Write a block of bytes to the sensor to configure it to take a measurement
bool     vl53l1x_newDataReady(void); //Polls the measurement completion bit
uint16_t vl53l1x_getDistance(void); //Returns the results from the last measurement, distance in mm
uint16_t vl53l1x_getSignalRate(void); //Returns the results from the last measurement, signal rate
void     vl53l1x_setDistanceMode(uint8_t mode);//Defaults to long range
