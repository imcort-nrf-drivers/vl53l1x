...
static uint8_t sendbuf[137];
//uint8_t _distanceMode = 0;

static void vl53l1x_i2c_write(uint16_t reg_addr, uint8_t * data, uint8_t length)
{
		
		sendbuf[0] = reg_addr >> 8;
		sendbuf[1] = reg_addr;
		memcpy(sendbuf + 2, data, length);
	
		iic_send(VL53L1X_ADDR, sendbuf, length + 2, false);
	
}

static void vl53l1x_i2c_read(uint16_t reg_addr, uint8_t * data, uint8_t length)
{

		sendbuf[0] = reg_addr >> 8;
		sendbuf[1] = reg_addr;
	
		iic_send(VL53L1X_ADDR, sendbuf, 2, false);
		iic_read(VL53L1X_ADDR, data, length);
	
}

static uint16_t vl53l1x_i2c_read16(uint16_t reg_addr)
{
	
		uint8_t recvbuf[2];
	
		vl53l1x_i2c_read(reg_addr, recvbuf, 2);
	
		return (recvbuf[0] << 8) | recvbuf[1];

}

static uint8_t vl53l1x_i2c_read8(uint16_t reg_addr)
{
	
		uint8_t recvbuf[1];
	
		vl53l1x_i2c_read(reg_addr, recvbuf, 1);
	
		return recvbuf[0];

}

static void vl53l1x_i2c_write16(uint16_t reg_addr, uint16_t data)
{
		...
		iic_send(VL53L1X_ADDR, sendbuf, 4, false);

}

static void vl53l1x_i2c_write8(uint16_t reg_addr, uint8_t data)
{
	
		vl53l1x_i2c_write(reg_addr, &data, 1);
...
{
  vl53l1x_i2c_write8(VL53L1_SOFT_RESET, 0x00); //Reset
  delay(1); //Driver uses 100us
  vl53l1x_i2c_write8(VL53L1_SOFT_RESET, 0x01); //Exit reset
}

//Check to see if sensor is responding
//Set sensor up for 2.8/3.3V I2C
//Return true if succesful
bool vl53l1x_begin(void)
{

  //Check the device ID
  uint16_t modelID = vl53l1x_i2c_read16(VL53L1_IDENTIFICATION__MODEL_ID);
  ...
  int counter = 0;
  while ((vl53l1x_i2c_read8(VL53L1_FIRMWARE__SYSTEM_STATUS) & 0x01) == 0) {
    if (counter++ == 100) return false; //Sensor timed out
    delay(10);
  }

  //Set I2C to 2.8V mode. In this mode 3.3V I2C is allowed.
  uint16_t result = vl53l1x_i2c_read16(VL53L1_PAD_I2C_HV__EXTSUP_CONFIG);
  result = (result & 0xFE) | 0x01;
  vl53l1x_i2c_write16(VL53L1_PAD_I2C_HV__EXTSUP_CONFIG, result);

  //Gets trim resistors from chip
//  for (uint16_t i = 0; i < 36; i++) {
//	  uint8_t regVal = vl53l1x_i2c_read8(i + 1);
//	  configBlock[i] = regVal;
//  }
	vl53l1x_i2c_read(1, configBlock, 36);
  vl53l1x_startMeasurement(0);
...
void vl53l1x_startMeasurement(uint8_t offset)
{
  uint16_t address = 1 + offset; //Start at memory location 0x01, add offset
	uint8_t leftToSend = sizeof(configBlock) - offset;
	
	vl53l1x_i2c_write(address, configBlock + offset, leftToSend);
	
//	for (uint16_t i = offset; i < 135; i++) {
////	  uint8_t regVal = vl53l1x_i2c_read8(i + 1);
////	   = regVal;
//		vl53l1x_i2c_write8(i + 1, configBlock[i]);
//		delay(1);
//  }
...
bool vl53l1x_newDataReady(void)
{
  if (vl53l1x_i2c_read8(VL53L1_GPIO__TIO_HV_STATUS) != 0x03) return(true); //New measurement!
...
{
  return vl53l1x_i2c_read16(VL53L1_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0);
}

//Get signal rate
//This seems to be a number representing the quality of the measurement, the number of SPADs used perhaps
uint16_t vl53l1x_getSignalRate()
{
  //From vl53l1_api.c line 2041
  uint16_t reading = vl53l1x_i2c_read16(VL53L1_RESULT__PEAK_SIGNAL_COUNT_RATE_CROSSTALK_CORRECTED_MCPS_SD0);// << 9; //FIXPOINT97TOFIXPOINT1616
...
void vl53l1x_setDistanceMode(uint8_t mode)
{
	uint8_t periodA;
	uint8_t periodB;
	uint8_t phaseHigh;
	...
	//timing
	vl53l1x_i2c_write8(VL53L1_RANGE_CONFIG__VCSEL_PERIOD_A, periodA);
	vl53l1x_i2c_write8(VL53L1_RANGE_CONFIG__VCSEL_PERIOD_B , periodB);
	vl53l1x_i2c_write8(VL53L1_RANGE_CONFIG__VALID_PHASE_HIGH, phaseHigh);
	
	//dynamic
	vl53l1x_i2c_write8(VL53L1_SD_CONFIG__WOI_SD0 , periodA);
	vl53l1x_i2c_write8(VL53L1_SD_CONFIG__WOI_SD1, periodB);
	vl53l1x_i2c_write8(VL53L1_SD_CONFIG__INITIAL_PHASE_SD0, phaseInit);
	vl53l1x_i2c_write8(VL53L1_SD_CONFIG__INITIAL_PHASE_SD1, phaseInit);
	
	//_distanceMode = mode;
}

//uint8_t vl53l1x_getDistanceMode()
//{
...
//This does that.
//uint8_t vl53l1x_getRangeStatus()
//{
